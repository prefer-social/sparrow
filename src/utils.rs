...
pub async fn get_actor_url_from_id(id: String) -> Result<String> {
    ...
    let webfinger_url = format!("https://{host}/.well-known/webfinger?resource=acct:{id}");
...
pub async fn get_privatekey_with_actor_url(actor_url: &str) -> Result<String> {
    let qr = db::Connection::builder().await.execute(
    "SELECT privateKey FROM signing_key JOIN user ON user.id = signing_key.userId WHERE user.federationId = ?", 
    &[SV::Text(actor_url.to_string())]).await;
    let private_key = qr.rows().next().unwrap().get::<&str>("privateKey").unwrap();
    Ok(private_key.to_string())
}
...
pub async fn get_public_key(actor_url_str: &str) -> Result<String> {
    ...
    let public_key = json
        .get("publicKey")
        .expect(format!("({status:?}) publicKey not found from: {body:?}").as_str());
    let public_key_id = public_key.get("id").unwrap();
    ...
    if actor_url_str == public_key_id {
        return Ok(public_key_pem);
    }
    // TODO: This shoud not return Err but Ok(None)
    Err(anyhow::Error::msg("key_id not matched"))
}
